...
namespace Unicord.Universal.Models.Messaging
{
    internal class DiscordClientMessenger
    {
        ...
        private static async Task<Task> OnResumed(ReadyEventArgs e)
        {
            return Task.WhenAll(WeakReferenceMessenger.Default.Send(e));
        }

        private static async Task<Task> OnRelationshipAdded(RelationshipEventArgs e)
        {
            return Task.WhenAll(WeakReferenceMessenger.Default.Send(e));
        }

        private static async Task<Task> OnRelationshipRemoved(RelationshipEventArgs e)
        {
            return Task.WhenAll(WeakReferenceMessenger.Default.Send(e));
        }
